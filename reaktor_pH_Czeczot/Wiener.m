    ...
    methods
        ...
        function testLinearModel(obj, U, a, b, delay, kk, Tp, rk4, index)
            ...
            legend('RK4', ...
                ...
                sprintf('Wiener (liniowy TS) \t E = %.3f', E_out), 'Location', 'best');
            title('Porównanie wyjścia układu rzeczywistego i modelu');
            ylabel('y [cm]');
            xlabel('t [s]');
            grid on;

            % saveas(gcf, sprintf('D:/EiTI/MGR/raporty/raport_luty/pictures/WienerLinearModel_%d.png', index));  % Zapisuje jako plik PNG
            % saveas(gcf, sprintf('D:/EiTI/IV sem/SDM II/WienerLinearModel_%d.png', index));  % Zapisuje jako plik PNG
        end
        ...
        function testNonlinearModel(obj, U, a, b, delay, kk, Tp, rk4, index)
            ...
            legend('RK4', ...
                sprintf('RK4 liniowy \t E = %.3f', E_lin), ...
                sprintf('Wiener (nieliniowy TS) \t E = %.3f', E_out), 'Location', 'best');
            title('Porównanie wyjścia układu rzeczywistego i modelu');
            ylabel('y [cm]');
            xlabel('t [s]');
            grid on;

            % saveas(gcf, sprintf('D:/EiTI/MGR/raporty/raport_luty/pictures/WienerNonlinearModel_%d.png', index));  % Zapisuje jako plik PNG
            % saveas(gcf, sprintf('D:/EiTI/IV sem/SDM II/WienerNonlinearModel_%d.png', index));  % Zapisuje jako plik PNG
        end
        ...
        function show_fuzzy(~, fis, s)
            ...
            % saveas(gcf, sprintf('D:/EiTI/MGR/raporty/raport_luty/pictures/WienerfuzzySets_%s.png', s));  % Zapisuje jako plik PNG
            % saveas(gcf, sprintf('D:/EiTI/IV sem/SDM II/WienerfuzzySets_%s.png', s));  % Zapisuje jako plik PNG
        end
