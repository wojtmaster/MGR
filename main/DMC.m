classdef DMC < handle
    ...
    methods
        ...
        function [y_mod, u, E] = dmc_analitic(obj, y_zad, u_D, a, b, delay, kk)
            ...
            E = sum((y_zad - y_mod).^2) + obj.lambda .* sum(delta_u.^2);
        end
        ...
        function [y_mod, u, E] = dmc_FUZZY(obj, y_zad, u_D, a, b, delay, kk, fis, F_10, F_D0, linearization)
            ...
            for k = delay+3:kk
                ...
                for i = 1:length(strength)
                    ...
                    % Obliczenie przyrostu sterowania dla chwili (i+1) w chwili i-tej
                    options = optimoptions('quadprog', 'Display', 'off');
                    % disp(["size(delta_uk) = ", num2str(size(delta_uk))]);
                    % temp = strength(i).*quadprog(H,f,A,B,[],[],delta_U_min,delta_U_max,[],options);
        ...
        function show_result(~, y_mod, y_zad, u, E, kk, s)
