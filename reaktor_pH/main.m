...
%% Dane do wykresów 3D
% Zakresy sterowania
Q_1 = -15:0.5:15;  % <- Można zmniejszyć gęstość na początek
Q_3= -15:0.5:15;
Q_2 = 0;

% Przygotuj siatkę
[U_1, U_3] = meshgrid(Q_1, Q_3);
h = zeros(size(U_1));
pH = zeros(size(U_1));
...
for i = 1:numel(U_1)
    ...
    u = [ones(1, obiekt.kk)*u_1;
         ones(1, obiekt.kk)*u_2;
         ones(1, obiekt.kk)*u_3];

    [y, ~, ~] = obiekt.modifiedEuler(u, obiekt.kk);

    % Zakładamy, że y(2, end) to końcowa wartość pH po sprowadzeniu do pH_0
    h(i) =  y(1, end) + obiekt.h_0;
    pH(i) = y(2, end) + obiekt.pH_0;  % <- dodaj pH_0 jeśli wcześniej odejmowałeś
end
