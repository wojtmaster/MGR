...
% Definiowanie wyjścia i początkowych następników (a_i * y + b_i)
fis = addOutput(fis, [Y_min Y_max], 'Name', 'H2');
...
% Dodanie reguł TS w postaci liniowej
for i = 1:length(Y_center)
    fis = addMF(fis, 'H2', 'linear', [a_param(i), b_param(i)]);
end
...
optimal_params = fminsearch(@(params) objective_function(params, fis, U, Y_real, obiekt, Y_center, a, b), initial_params, options);

% Po optymalizacji
a_optimal = optimal_params(1:length(Y_center));
b_optimal = optimal_params(length(Y_center)+1:end);
...
% Początkowe współczynniki (a_i, b_i)
a_fmin = [62.9491   81.3615   34.6719]; % Można dostroić
b_fmin = [84.8279   74.0518   30.5659];

a_param = [44.1597 47.1854 50.7124]; % Można dostroić
b_param = [59.9872 44.9984 45.1212];
...
fis_2 = fis;
for i = 1:length(Y_center)
    fis_2.Outputs.MembershipFunctions(i).Parameters(1) = a_fmin(i);
    fis_2.Outputs.MembershipFunctions(i).Parameters(2) = b_fmin(i);
end
...
for j = 1:5
    ...
    for k = obiekt.delay+3:obiekt.kk
        ...
        % y_out(k) = max(Y_min, min(Y_max, y_out(k)));
        [~, degrees] = evalfis(fis, y_out(k)); % Przepuszczenie przez model TS
        [~, degrees_2] = evalfis(fis_2, y_out(k)); % Przepuszczenie przez model TS
        output = 0;
        output_2 = 0;
        for i = 1:length(a_param)
            output = output + degrees(i) * (a_param(i) * sinh(y_out(k) / b_param(i)));
            output_2 = output_2 + degrees_2(i) * (a_fmin(i) * sinh(y_out(k) / b_fmin(i)));
        end
        Y_out(k) = output / sum(degrees);
        Y_out_2(k) = output_2 / sum(degrees_2);
        % Y_out(k) = max(Y_min, min(Y_max, Y_out(k)));
    end
...
function E_out = objective_function(params, fis, U, Y_real, obiekt, Y_center, a, b)
    % Parametry do optymalizacji
    a_param = params(1:length(Y_center));  % a_param
    b_param = params(length(Y_center)+1:end);  % b_param
