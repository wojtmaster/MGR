...
U = [repelem([0, -15, 15, 45, 75], 250);
    zeros(1,obiekt.kk);
    zeros(1,obiekt.kk)];

[Y_real, Y_lin] = obiekt.modifiedEuler(U, obiekt.kk); % Symulacja rzeczywistego układu

% U_train = zeros(5, obiekt.kk);
% Y_train = zeros(5, obiekt.kk);

% for i = 1:5
%     U_tmp = [repelem((rand(1, obiekt.kk/250) * 30 - 15), 250);
%              zeros(1, obiekt.kk);
%              zeros(1, obiekt.kk)];
...
for i = 1:length(U_center)
    fis = addMF(fis, 'Q1_fuzzy', 'linear', [a_param(i), b_param(i)]);
end
...
optimal_params = fminsearch(@(params) objective_function(params, fis, U(1,:), Y_real(1,:), obiekt, U_center, a, b), initial_params, options);
...
figure;
plotmf(fis, 'input', 1);
title('Funkcje przynależności dla Q1');
grid on;

U = [repelem((rand(1, obiekt.kk/250) * 30 - 15), 250);
    zeros(1,obiekt.kk);
    zeros(1,obiekt.kk)];
[Y_real, Y_lin] = obiekt.modifiedEuler(U, obiekt.kk); % Symulacja rzeczywistego układu

U_fuzzy = evalfis(fis, U(1,:));
Y_out = zeros(1, obiekt.kk);

for k = 4:obiekt.kk
    Y_out(k) = - a*[Y_out(k-1:-1:k-3)]' + b*U_fuzzy(k-1:-1:k-3);
end
...
% function E_out = objective_function(params, fis, U, Y_real, obiekt, U_center, a, b)
%     % Parametry do optymalizacji
%     a_param = params(1:length(U_center));
%     b_param = params(length(U_center)+1:end);
% 
...
% 
%     % Inicjalizacja błędu
%     [M, N] = size(U);  % M = liczba trajektorii
%     E_out = 0;
% 
%     for traj = 1:M
%         u_single = U(traj, :);  % Jedna trajektoria wejściowa
%         y_real_single = Y_real(traj, :);  % Odpowiadające wyjście
% 
%         u_fuzzy = evalfis(fis, u_single');  % N × 1
%         y_out = zeros(1, N);
% 
%         for k = 4:N
%             y_out(k) = -a * y_out(k-1:-1:k-3)' + b * u_fuzzy(k-1:-1:k-3);
...
% 
% %Generacja danych sterujących i RK4
% U = [repelem([0, -7.5, -15, 7.5, 15], 250);
%     zeros(1,obiekt.kk);
%     zeros(1,obiekt.kk)];
% 
% [Y_real, Y_lin] = obiekt.modifiedEuler(U, obiekt.kk); % Symulacja rzeczywistego układu
% 
% U_fis = U(1,:)';                 % tylko pierwszy sygnał sterujący (zakładam, że to główny)
% Y_fis_target = Y_real(1,:)' - Y_lin(1,:)'; % nieliniowa część (różnica między real a liniowy)
% 
% data = [U_fis, Y_fis_target];  % kolumny: wejście, wyjście
% 
% % Automatyczne generowanie FIS-a z 5 zbiorami rozmytymi
% numMFs = 5;
% mfType = 'gaussmf';
% 
% in_fis = genfis1(data, numMFs, mfType, 'linear');  % tworzy gotowy FIS z regułami Sugeno
% 
% % Trening
% [trainedFIS, trainError] = anfis(data, in_fis, 50);
% 
% %%
% 
% U = [repelem((rand(1, obiekt.kk/250) * 30 - 15), 250);
...
% [Y_real, Y_lin] = obiekt.modifiedEuler(U, obiekt.kk); % Symulacja rzeczywistego układu
% 
% U_pred = evalfis(trainedFIS, U(1,:));  % FIS przekształca U
% Y_model = Y_lin(1,:)' + U_pred;            % efekt końcowy systemu Hammersteina
% 
% % Porównanie
% figure;
% plot(Y_real(1,:)', 'k'); hold on;
% plot(Y_model, 'r--');
% legend('Y_{real}', 'Y_{Hammerstein}');
% title('Porównanie systemu rzeczywistego i modelu Hammersteina');
