...
for i = (N+delay)+1:kk/2
    ...
    for j = 1:N
        ...
        for l = k:M
            M_temp_ucz = [M_temp_ucz, u_ucz(1, i-(delay+j)).^l];
            M_temp_wer = [M_temp_wer, u_wer(1, i-(delay+j)).^l];
        end
    end
    for j = 1:N
        for k = 1:M
...
for k = N+delay+1:kk/2
    ...
    for i = 1:N
        ...
        for j = m:M
            Y_temp = [Y_temp, y_ucz(k-i)^j];
            Z_temp = [Z_temp, z(k-(delay+i))^j];
            D_temp = [D_temp, u_ucz(2, k-i)^j];
        end
    end
    % y_mod_ucz(k) = - a * [y_ucz(k-1:-1:k-N)]' + b(:, 1)'*[z(k-(delay+1):-1:k-(delay+N))]' + b(:,2)'*[u_ucz(2, k-1:-1:k-N)]';
    y_mod_ucz(k) = - a*Y_temp' + b(:,1)'*Z_temp' + b(:,2)'*D_temp';
end
...
for k = N+delay+1:kk/2
    ...
    for i = 1:N
        ...
        for j = m:M
            Y_temp = [Y_temp, y_wer(k-i)^j];
            Z_temp = [Z_temp, z(k-(delay+i))^j];
            D_temp = [D_temp, u_wer(2, k-i)^j];
        end
    end
    % y_mod_wer(k) =  - a * [y_wer(k-1:-1:k-N)]' + b(:, 1)'*[z(k-(delay+1):-1:k-(delay+N))]' + b(:,2)'*[u_wer(2, k-1:-1:k-N)]';
    y_mod_wer(k) = - a*Y_temp' + b(:,1)'*Z_temp' + b(:,2)'*D_temp';
end
