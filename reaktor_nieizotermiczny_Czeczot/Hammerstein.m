classdef Hammerstein < handle
    ...
    methods
        ...
        function testLinearModel(obj, U, a, b, delay, kk, Tp, rk4, index)
            ...
            legend('RK4', ...
                sprintf('RK4 liniowy \t E = %.3f', E_lin), ...
                sprintf('Hammerstein (liniowy TS) \t E = %.3f', E_out), 'Location', 'best');
            title('Porównanie wyjścia układu rzeczywistego i modelu');
            ylabel('y [cm]');
            xlabel('t [s]');
            grid on;

            % saveas(gcf, sprintf('D:/EiTI/MGR/raporty/raport_luty/pictures/HammersteinLinearModel_%d.png', index));  % Zapisuje jako plik PNG
            % saveas(gcf, sprintf('D:/EiTI/IV sem/SDM II/HammersteinLinearModel_%d.png', index));  % Zapisuje jako plik PNG
        end
        ...
        function testNonlinearModel(obj, U, a, b, delay, kk, Tp, rk4, index)
            ...
            legend('RK4', ...
                sprintf('RK4 liniowy \t E = %.3f', E_lin), ...
                sprintf('Hammerstein (nieliniowy TS) \t E = %.3f', E_out), 'Location', 'best');
            title('Porównanie wyjścia układu rzeczywistego i modelu');
            ylabel('y [cm]');
            xlabel('t [s]');
            grid on;

            % saveas(gcf, sprintf('D:/EiTI/MGR/raporty/raport_luty/pictures/HammersteinNonlinearModel_%d.png', index));  % Zapisuje jako plik PNG
            % saveas(gcf, sprintf('D:/EiTI/IV sem/SDM II/HammersteinNonlinearModel_%d.png', index));  % Zapisuje jako plik PNG
        end

        function show_fuzzy(~, fis, s)
            figure;
            plotmf(fis, 'input', 1);
            title(sprintf('Funkcje przynależności dla U - następniki %s', s));
            xlabel('u');
            ylabel('$\mu(u)$', 'Interpreter', 'latex');
            grid on;

            % saveas(gcf, sprintf('D:/EiTI/MGR/raporty/raport_luty/pictures/HammersteinfuzzySets_%s.png', s));  % Zapisuje jako plik PNG
            % saveas(gcf, sprintf('D:/EiTI/IV sem/SDM II/HammersteinfuzzySets_%s.png', s));  % Zapisuje jako plik PNG
        end
    end
end