function [] = diff_eq_wiener_OE(a, b, u_ucz, u_wer, y_ucz, y_wer, params, F_10, h_20, F_1_start, F_1_end, R, kk, tau, n)
    ...
    % Zbiór uczący
    for k = tau+3:kk/2
        v(k) = - a(1)*y_mod_ucz(k-1) - a(2)*y_mod_ucz(k-2) + b(tau+1)*(u_ucz(1,k-(tau+1)) + u_ucz(2, k-1)) + b(tau+2)*(u_ucz(1,k-(tau+2)) + u_ucz(2, k-2));
    ...
    % Zbiór weryfikujący
    for k = tau+3:kk/2
        v(k) = - a(1)*y_mod_wer(k-1) - a(2)*y_mod_wer(k-2) + b(tau+1)*(u_wer(1,k-(tau+1)) + u_wer(2, k-1)) + b(tau+2)*(u_wer(1,k-(tau+2)) + u_wer(2, k-2));
        index = round((v(k)+F_10-F_1_start)*n / (F_1_end-F_1_start));
