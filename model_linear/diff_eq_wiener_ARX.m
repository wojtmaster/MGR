function [] = diff_eq_wiener_ARX(a, b, K, u_ucz, u_wer, y_ucz, y_wer, params, v_start, v_end, R, kk, tau, n)
    ...
    % Zbiór uczący
    for k = tau+3:kk/2
        v_ucz(k) = - a'*[y_ucz(k-1:-1:k-2)]' + (b(tau+1:tau+2)/K)'*[u_ucz(1,k-(tau+1):-1:k-(tau+2))]' + b(tau+1:tau+2)'*[u_ucz(2, k-1:-1:k-2)]';
    ...
    % Zbiór weryfikujący
    for k = tau+3:kk/2
        v_wer(k) = - a'*[y_wer(k-1:-1:k-2)]' + (b(tau+1:tau+2)/K)'*[u_wer(1,k-(tau+1):-1:k-(tau+2))]' + b(tau+1:tau+2)'*[u_wer(2, k-1:-1:k-2)]';
