function [v] = diff_eq_OE(a, b, u_ucz, u_wer, y_ucz, y_wer, kk, tau)
    ...
    % Zbiór uczący
    for k = tau+3:kk/2
        y_mod_ucz(k) = - a(1)*y_mod_ucz(k-1) - a(2)*y_mod_ucz(k-2) + b(tau+1)*(u_ucz(1, k-(tau+1)) + u_ucz(2, k-1)) + b(tau+2)*(u_ucz(1, k-(tau+2)) + u_ucz(2, k-2));
    end

    % Zbiór weryfikujący
    for k = tau+3:kk/2
        y_mod_wer(k) = - a(1)*y_mod_wer(k-1) - a(2)*y_mod_wer(k-2) + b(tau+1)*(u_wer(1, k-(tau+1)) + u_wer(2, k-1)) + b(tau+2)*(u_wer(1, k-(tau+2)) + u_wer(2, k-2));
    end

    v = [y_mod_ucz, y_mod_wer];
    ...
    legend('y_{mod}', 'y');
    grid on;
    % file_name = sprintf('../raport/pictures/oe_ucz.pdf');
    ...
    legend('y_{mod}', 'y');
    grid on;
    % file_name = sprintf('../raport/pictures/oe_wer.pdf');
    % exportgraphics (gcf, file_name);        
