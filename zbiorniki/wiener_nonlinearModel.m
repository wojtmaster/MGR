...
% a_param = ones(1,3)*18;
% b_param = ones(1,3)*1.9;

a_param = [5.7627 16.2006 12.0295];
b_param = [-10.9354 -0.5527 15.2669];
...
for i = 1:length(h)
    for j = 1:length(h)
        ...
        for k = 1:rules_number
            output = output +  degrees(k) * (a_optimal(k)*sinh(h(i,j)/36) + b_optimal(k));
            w = w + degrees(k);
        end
...
for k = 8:obiekt.kk
    ...
    for i = 1:rules_number
        output = output +  degrees(i) * (a_optimal(i)*sinh(Y(k)/36) + b_optimal(i));
        w = w + degrees(i);
    end
    Y_fuzzy(k) = output / w;
end
...
function E_out = nonlinearCoeff(params, sigma, center, U, Y, rules_number)
    ...
    for i = 1:length(U)
        for j = 1:length(U)
            ...
            output = 0;
            for k = 1:rules_number
                output = output + degrees(k)*(a_param(k)*sinh(U(i,j)/36) + b_param(k));
                w = w + degrees(k);
            end
            Y_out(i,j) = output / w;
