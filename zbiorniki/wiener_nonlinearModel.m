...
% options = optimset('Display', 'iter', 'MaxFunEvals', 2000, 'MaxIter', 1000); % Opcje optymalizacji
% optimal_params = fminsearch(@(params) linearCoeff(params, sigma, h, Y, length(fis.Rules)), initial_params, options);
...
for k = 1:length(U)
    output = 0;
    w = 0;
    degrees = [gaussmf_val(h(k), sigma, Y_center(1)), ...
               gaussmf_val(h(k), sigma, Y_center(2)), ...
               gaussmf_val(h(k), sigma, Y_center(3))];
...
for k = 8:obiekt.kk
    ...
    for i = 1:length(fis.Rules)
        output = output +  degrees(i) * (a_optimal(i)*sinh(Y(k)/b_optimal(i)) + c_optimal(i));
        w = w + degrees(i);
    end
