...
for i = 1:length(U_center) 
    fis = addMF(fis, 'U', 'gaussmf', [sigma, U_center(i)]);
end
...
optimal_params = fminsearch(@(params) nonlinearCoeff(params, sigma, U_center, U, Y, length(fis.Rules)), initial_params, options);

% Po optymalizacji
% a_optimal = optimal_params(1:3);
% b_optimal = optimal_params(4:6);
% c_optimal = optimal_params(7:end);
a_optimal = a_param;
b_optimal = b_param;
c_optimal = c_param;
...
for k = 1:length(U)
    output = 0;
    w = 0;
    degrees = [gaussmf_val(U(k), sigma, U_center(1)), ...
               gaussmf_val(U(k), sigma, U_center(2)), ...
               gaussmf_val(U(k), sigma, U_center(3))];
    for i = 1:length(fis.Rules)
        output = output +  degrees(i) * (a_optimal(i)*sinh(U(k)/b_optimal(i)) + c_optimal(i));
        w = w + degrees(i);
    end
    Y_out(k) = output / w;
end

plot(t, Y, 'b-', t, Y_out, 'ro');
grid on;
...
output = 0;
w = 0;
degrees = [gaussmf_val(0, sigma, U_center(1)), ...
           gaussmf_val(0, sigma, U_center(2)), ...
           gaussmf_val(0, sigma, U_center(3))];
for i = 1:length(fis.Rules)
    output = output +  degrees(i) * (a_optimal(i)*sinh(0/b_optimal(i)) + c_optimal(i));
    w = w + degrees(i);
end
Y_0 = output / w;
...
for k = 1:7
    output = 0;
    w = 0;
    degrees = [gaussmf_val(U(1,k), sigma, U_center(1)), ...
               gaussmf_val(U(1,k), sigma, U_center(2)), ...
               gaussmf_val(U(1,k), sigma, U_center(3))];
    for i = 1:length(fis.Rules)
        output = output +  degrees(i) * (a_optimal(i)*sinh(U(1,k)/b_optimal(i)) + c_optimal(i));
        w = w + degrees(i);
    end
    Y_fuzzy(k) = output / w;
end
...
function E_out = nonlinearCoeff(params, sigma, U_center, U, Y, rules_number)
    gaussmf_val = @(x, sigma, c) exp(-((x - c).^2) / (2 * sigma^2));
    ...
    for k = 1:length(U)
        ...
        for i = 1:rules_number
            output = output +  degrees(i) * (a_param(i)*sinh(U(k)/b_param(i)) + c_param(i));
            w = w + degrees(i);
        end
        Y_out(k) = output / w;
