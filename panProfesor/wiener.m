...
for k = 1:D
    if (k == 1)
        s(k) = b(1);
    elseif (k == 2)
        s(k) = -a(1)*s(k-1) + b(1) + b(2);
    else
        s(k) = -a(1)*s(k-1) - a(2)*s(k-2) + b(1) + b(2);    
    end
end
b(2) = 0;
%% DMC - kontynuacja
...
%% Symulacja obiektu - wegenerowana sekwencja sygnału sterującego 
clear v;
clear y;
clear u;

u.sim = zeros(1, 250);
% Narastanie eksponencjalne do ok. 1.1 (do k = 120)
for k = 1:125
    u.sim(k) = 1.1 * (1 - exp(-0.025 * (k - 1)));
end

% Gwałtowny spadek po k = 120
for k = 126:250
    u.sim(k) = u.sim(125) * exp(-0.1 * (k - 125));
end

% Wykres
figure;
plot(0:249, u.sim, 'k', 'LineWidth', 1.5); % czarna linia ciągła
xlabel('time');
ylabel('u_k');
grid on;
...
v.sim = zeros(1,250);
y.sim = zeros(1,250);
y.wiener = zeros(1,250);
for k = 3:250
    v.sim(k) = -a(1)*v.sim(k-1) - a(2)*v.sim(k-2) + b(1)*u.sim(k-1) + b(2)*u.sim(k-2);
    y.sim(k) = 0.3163*v.sim(k) / sqrt(0.1 + 0.9*v.sim(k)^2);
    y.wiener(k) = evalfis(fis, v.sim(k));
end
...
function y_0 = fuzzy_free_response(v, u, y, k, a, b, N, fis)
    ...
    for i = 1:N
        if i == 1
            v_0(i) = -a(1)*v(k) - a(2)*v(k-1) + b(1)*u_0 + b(2)*u_0;
        elseif i == 2
            v_0(i) = -a(1)*v_0(i-1) - a(2)*v(k) + b(1)*u_0 + b(2)*u_0;
        else
